    ...
    public class ApplyMaintainModule : IDisposable
    {
        ...
        public ApplyMaintainListResultModel DoGetList(ApplyMaintainListFilterModel filterModel)
        {
            ...
            try
            {
                ...
                result.Data = data;
                foreach (var d in data)
                    d.ApplyStatus = ActivityStatusCheckByID(d.ID);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        /// <param name="data"></param>
        private void ListPageList(int currentPage, ref List<ApplyMaintainListDataModel> data, out PaginationResult pagination)
        {
